...
export function uploadFileTool( server: McpServer ): RegisteredTool {
	return server.tool(
		...
		async (
			{ wikiSite, filepath, title, text, comment }
		) => handleUploadFileTool( wikiSite, filepath, title, text, comment )
	);
}

async function handleUploadFileTool(
	wikiSite: string, filepath: string, title: string, text: string, comment?: string
